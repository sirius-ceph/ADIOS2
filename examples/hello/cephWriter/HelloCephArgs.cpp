 ...
 *  Created on: 
 *      Author: 
 *  Adapted from HelloSkeletonArgs.cpp
 */
...
static void printUsage(bool isWriter)
{
    ...
    {
        std::cout
            << "Usage: Usage: helloCephWriter  config  N  M   nx  ny   "
               "steps  sleeptime\n";
    }
    else
    {
        std::cout << "Usage: Usage: helloCephReader  config  N  M \n  ";
    }
...
// Converts user command line args to int vals.
...
HelloCephArgs::HelloCephArgs(bool isWriter, int argc, char *argv[],
                                     int rank, int nproc)
: rank{rank}
{
...
void HelloCephArgs::DecomposeArray(int NX, int NY)
{
    gndx = NX;
    gndy = NY;
    posx = rank % npx;
